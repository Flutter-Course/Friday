...
class FirebaseHandler {
  ...
  Future<void> addFavorite(Movie movie, User user) async {
    String url =
        'https://roovies-alex.firebaseio.com/users/${user.userId}/favorites/${movie.id}.json';
    final params = {
      'auth': user.idToken,
    };
    await _dio.put(
  ...
  Future<void> removeFavorite(Movie movie, User user) async {
    String url =
        'https://roovies-alex.firebaseio.com/users/${user.userId}/favorites/${movie.id}.json';
    final params = {
      'auth': user.idToken,
  ...
  Future<List<Movie>> getFavorites(User user) async {
    String url =
        'https://roovies-alex.firebaseio.com/users/${user.userId}/favorites.json';
    final params = {
      'auth': user.idToken,
    };
